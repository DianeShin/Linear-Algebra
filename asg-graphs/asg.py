...
def isPermutationMatrix(matrix):
    ...
    for row in range(np.shape(matrix)[0]):
        for column in range(np.shape(matrix)[1]):
            if matrix[row][column] != 0 and matrix[row][column] != 1:
                raise ValueError

    # check condition
    rowSum = np.sum(matrix, axis = 1)
    columnSum = np.sum(matrix, axis = 0)
    if (np.array_equal(rowSum, np.ones(np.shape(matrix)[0]))and np.array_equal(columnSum, np.ones(np.shape(matrix)[1]))):
        return True
    else:
        return False
...
def allPermutationMatrices(n):
    ...
    for i in range(n-1):
        initArray.append(0)
    rowList = list(dict.fromkeys(itertools.permutations(initArray)))
    matrixList = list(dict.fromkeys(itertools.permutations(rowList)))

    result = []
    for i in range(len(matrixList)):
        result.append(np.array(matrixList[i]))
...
def isIsomorphicUsingP(A, B, P):
    ...
    """
    # check dimension
    if (np.shape(A) != np.shape(B) != np.shape(P)):
        raise ValueError
    ...
    # calculate
    return np.array_equal(A,np.matmul(P,np.matmul(P, B).T))  
...
def numIsomorphisms(A, B):
    ...
    0
    """
    permList = allPermutationMatrices(np.shape(A)[0])
    count = 0
    for P in permList:
