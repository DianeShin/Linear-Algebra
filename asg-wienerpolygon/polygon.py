...
def norm(x):
    ...
    5.0
    """
    return math.sqrt(np.sum(np.array([num ** 2 for num in x])))
...
def initPoints(num):
    ...
    np.random.seed(234)
    xPoint = np.random.rand(num-1)
    np.random.seed(6786)
    yPoint = np.random.rand(num-1)
    xPoint = np.append(xPoint, np.sum(xPoint) * -1)
    yPoint = np.append(yPoint, np.sum(yPoint) * -1)
    return (xPoint / norm(xPoint), yPoint / norm(yPoint))
...
def invMatrixM(M):
    ...
    array([[ 1., -1.,  1.],
           ...
           [-1.,  1.,  1.]])
    """
    if (np.shape(M)[0] % 2 == 0 or np.shape(M)[1] % 2 == 0):
        raise ValueError
...
def updatePoints(M, x, y):
    ...
    """
    if not math.isclose(norm(x) , 1, rel_tol= 1e-6) or not math.isclose(norm(y) , 1, rel_tol= 1e-6):
        return ValueError

    xPoint = np.matmul(M, x)
    xPoint = xPoint / norm(xPoint)
    yPoint = np.matmul(M, y)
    yPoint = yPoint / norm(yPoint)
...
def pairCS(num):
    ...
    tau = np.array([2 * n for n in range(num)]) * math.pi / num
    c = math.sqrt(2/num) * np.array([math.cos(x) for x in tau])
    s = math.sqrt(2/num) * np.array([math.sin(x) for x in tau])
    return (c,s)
...
def convEllipse(x, y):
    ...
    c, s = pairCS(np.shape(x)[0])
    cos_u = (c.T @ x) / math.sqrt((c.T @ x) ** 2 + (s.T @ x) ** 2)
    sin_u = (s.T @ x) / math.sqrt((c.T @ x) ** 2 + (s.T @ x) ** 2)
    cos_v = (c.T @ y) / math.sqrt((c.T @ y) ** 2 + (s.T @ y) ** 2)
    sin_v = (s.T @ y) / math.sqrt((c.T @ y) ** 2 + (s.T @ y) ** 2)
    
    return np.array([cos_u * c + sin_u * s, cos_v * c + sin_v * s]). T
