    ...
    if A is None:
        ...
        for row in range(A.size[0]):
            for column in range(A.size[1]):
                if (A.store[row][column] != B.store[row][column]):
                    return False
...
def add(A, B):
    ...
    for row in range(A.size[0]):
        for column in range(A.size[1]):
            resultStore[row].append(A.store[row][column] + B.store[row][column])

    return Mat(resultStore)
...
def scalar_mul(M, x):
    ...
    """
    resultStore = []
    for row in range(M.size[0]):
        resultStore.append([])

    for row in range(M.size[0]):
        for column in range(M.size[1]):
            resultStore[row].append(M.store[row][column] * x)
...
def transpose(M):
    ...
    for column in range(M.size[1]):
        for row in range(M.size[0]):
            resultStore[column].append(M.store[row][column])
    return Mat(resultStore)
...
def vector_matrix_mul(v, M):
    ...
    resultStore = []
    for columnMatrix in range(M.size[1]):
        sum = 0
        for columnVector in range(v.size):
            sum += M.store[columnVector][columnMatrix] * v.store[columnVector]
        resultStore.append(sum)
    return Vec(resultStore)
...
def matrix_vector_mul(M, v):
    ...
    resultStore = []
    for rowMatrix in range(M.size[0]):
        sum = 0
        for rowVector in range(v.size):
            sum += M.store[rowMatrix][rowVector] * v.store[rowVector]
        resultStore.append(sum)
    return Vec(resultStore)
...
def matrix_matrix_mul(A, B):
    ...
    for ARow in range(A.size[0]):
        resultStore.append(vector_matrix_mul(Vec(A.store[ARow]), B).store)
    return Mat(resultStore)
