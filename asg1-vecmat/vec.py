...
def add(u, v):
    ...
    resultStore = []
    for i in range(v.size):
        resultStore.append(v.store[i] + u.store[i])
...
def scalar_mul(v, alpha):
    ...
    resultStore = []
    for i in range(v.size):
        resultStore.append(v.store[i] * alpha)
    result = Vec(resultStore)
    return result
