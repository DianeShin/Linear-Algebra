...
def distanceImages(dataImg1, dataImg2):
    ...
    Raises:
    -------
    TypeError : if dataImg1 or dataImg2 are not 1-dimensional array 
    ...
    sub = np.subtract(dataImg1, dataImg2)
    square = np.square(sub)
    result = math.sqrt(np.sum(square))
    return result
...
def projectAndBackProject(dataImg, mean, eigenVectors):
    ...
    340
    """
    result = cv2.PCAProject(dataImg, mean.reshape(dataImg.shape), eigenVectors)
    pic = cv2.PCABackProject(result, mean.reshape(dataImg.shape), eigenVectors).reshape(dataImg.shape)
    return pic
...
def indexDistancePairsSortedByDistance(data, r, mean, eigenVectors):
    ...
    calcResult = np.array([distanceImages(dataImg, projectAndBackProject(dataImg, mean, eigenVectors)) for dataImg in data])
    resultArray = zip(r , calcResult)
    result = sorted(list(resultArray), key = lambda x: x[1])
    return result
